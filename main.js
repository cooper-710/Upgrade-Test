...
function addBall(pitch, pitchType) {
  ...
  ball.userData = {
    timeToPlate: pitch.time_to_plate + 0.05,
...
function animate() {
  ...
  if (playing) {
    ...
    for (let ball of balls) {
      ...
      if (!ball.userData.frozen && (now - ball.userData.t0) >= ball.userData.timeToPlate) {
        ball.userData.frozen = true;
        ball.position.x = release.x + velocity.x * (now - t0) + 0.5 * accel.x * Math.pow((now - t0), 2);
        ball.position.y = release.y + velocity.y * (now - t0) + 0.5 * accel.y * Math.pow((now - t0), 2);
        ball.position.z = release.z + velocity.z * (now - t0) + 0.5 * accel.z * Math.pow((now - t0), 2);
      }
