...
function addBall(pitch, pitchType) {
  ...
  ball.userData = {
    ...
    // Adjust for correct animation duration
const actualT = pitch.time_to_plate || 0.45;
const expectedT = 0.45;
const scale = expectedT / actualT;
...
accel: {
  x: -pitch.ax * scale * scale,
  y: pitch.az * scale * scale,
  z: pitch.ay * scale * scale
},
...
function animate() {
  ...
  if (playing) {
    const trailOn = trailToggle.checked;
    ...
    for (let ball of balls) {
      ...
      if (trailOn) {
        ...
        trail.position.set(ball.position.x, ball.position.y, ball.position.z);
        scene.add(trail);
        setTimeout(() => scene.remove(trail), 500); // auto remove trail
      }
...
// === Metrics Panel & Trail Toggle ===
const metricsPanel = document.createElement('div');
metricsPanel.id = 'metricsPanel';
metricsPanel.style.position = 'absolute';
metricsPanel.style.top = '12px';
metricsPanel.style.right = '12px';
metricsPanel.style.background = 'rgba(0,0,0,0.6)';
metricsPanel.style.color = '#fff';
metricsPanel.style.padding = '12px';
metricsPanel.style.borderRadius = '8px';
metricsPanel.style.fontFamily = 'monospace';
metricsPanel.style.fontSize = '13px';
metricsPanel.style.maxWidth = '200px';
document.body.appendChild(metricsPanel);

const trailToggle = document.createElement('input');
trailToggle.type = 'checkbox';
trailToggle.id = 'trailToggle';
trailToggle.style.marginRight = '6px';

const trailLabel = document.createElement('label');
trailLabel.htmlFor = 'trailToggle';
trailLabel.textContent = 'Show Trails';
trailLabel.style.color = 'white';

const trailWrapper = document.createElement('div');
trailWrapper.style.position = 'absolute';
trailWrapper.style.bottom = '12px';
trailWrapper.style.left = '12px';
trailWrapper.style.background = 'rgba(0,0,0,0.5)';
trailWrapper.style.padding = '10px';
trailWrapper.style.borderRadius = '6px';
trailWrapper.appendChild(trailToggle);
trailWrapper.appendChild(trailLabel);
document.body.appendChild(trailWrapper);

const cameraSelect = document.createElement('select');
cameraSelect.id = 'cameraSelect';
['Behind Catcher','Side View','Overhead','Umpire View'].forEach(view => {
  const opt = document.createElement('option');
  opt.value = view;
...
cameraSelect.style.bottom = '12px';
cameraSelect.style.right = '12px';
cameraSelect.style.padding = '6px';
cameraSelect.style.background = '#222';
...
function showMetrics(pitch, pitchType) {
  const mph = Math.abs(pitch.vy0 * 0.681818).toFixed(1);
  const rpm = pitch.release_spin_rate || 0;
  const axis = pitch.spin_axis || 0;
  const entry = `<div><strong>${pitchType}</strong> | ${mph} mph | ${rpm} rpm | Axis: ${axis}°</div>`;
  const id = `metric-${pitchType.replace(/\s+/g, '-')}`;
...
function removeMetrics(pitchType) {
  const id = `metric-${pitchType.replace(/\s+/g, '-')}`;
...
cameraSelect.addEventListener('change', () => {
  ...
  if (view === 'Behind Catcher') {
    camera.position.set(0, 2.5, -65);
    camera.lookAt(0, 2.5, 0);
  } else if (view === 'Side View') {
    camera.position.set(15, 2.5, -40);
    camera.lookAt(0, 2.5, -60.5);
  } else if (view === 'Overhead') {
    camera.position.set(0, 40, -60.5);
    camera.lookAt(0, 2.5, -60.5);
  } else if (view === 'Umpire View') {
    camera.position.set(0, 5.5, -58);
    camera.lookAt(0, 2.5, -60.5);
  }
});
