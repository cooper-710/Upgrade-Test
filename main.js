...
function updateCamera(view) {
  switch(view) {
    case 'catcher': // Home‑plate / catcher view
      camera.position.set(0, 2.5, -65);
      camera.lookAt(0, 2.5, 0);
      break;

    case 'pitcher': // Pitcher's POV just behind the mound rubber
      camera.position.set(0, 2.8, 3);
      camera.lookAt(0, 2.5, -60.5);
      break;

    case 'rhh': // Right‑handed hitter box
      camera.position.set(1.6, 1.3, -63);
      camera.lookAt(0, 2.5, -60.5);
      break;

    case 'lhh': // Left‑handed hitter box
      camera.position.set(-1.6, 1.3, -63);
      camera.lookAt(0, 2.5, -60.5);
      break;

    case '1b': // First‑base dugout angle
      camera.position.set(20, 5, -55);
      camera.lookAt(0, 2.5, -60.5);
      break;

    case '3b': // Third‑base dugout angle
      camera.position.set(-20, 5, -55);
      camera.lookAt(0, 2.5, -60.5);
      break;

    default: // Fallback – same as catcher
      camera.position.set(0, 2.5, -65);
      camera.lookAt(0, 2.5, 0);
  }
}

/* Attach dropdown listener (if present) */
const camSelectEl = document.getElementById('cameraSelect');
if (camSelectEl) {
  camSelectEl.addEventListener('change', (e) => {
    updateCamera(e.target.value);
  });
  // Ensure default view matches initial option
  updateCamera(camSelectEl.value || 'catcher');
}
