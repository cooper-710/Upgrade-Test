...
function addBall(pitch, pitchType) {
  ...
  ball.userData = {
    timeToPlate: pitch.time_to_plate,
...
function animate() {
  ...
  if (playing) {
    ...
    for (let ball of balls) {
      ...
      if (!ball.userData.frozen && t >= ball.userData.timeToPlate) {
        ball.userData.frozen = true;
        ball.position.x = release.x + velocity.x * ball.userData.timeToPlate + 0.5 * accel.x * ball.userData.timeToPlate ** 2;
        ball.position.y = release.y + velocity.y * ball.userData.timeToPlate + 0.5 * accel.y * ball.userData.timeToPlate ** 2;
        ball.position.z = release.z + velocity.z * ball.userData.timeToPlate + 0.5 * accel.z * ball.userData.timeToPlate ** 2;
      }
